...
def get_stock(request):
    ...
    | Part Name | Total Stock                 | Defect Stock | Total Usable Stock         |
    |-----------|-----------------------------|--------------|----------------------------|
    | Part1     | Total Stock= inward-outward | Defect stock | =Total Stock - DefectStock |
...
class DomesticinvoiceUpdateView(
    LoginRequiredMixin,
    PermissionRequiredMixin,
    SuccessMessageMixin,
    UpdateView
    ...
    template_name = "inventory/domesticinvoice_update.html"
    permission_required = 'inventory.change_domesticinvoice'
    permission_denied_message = "User not allowed to this view."
...
@login_required
def get_saftey_stock(request):
    """
    Below is the Get Stock Format dispalyed in the UI.
    ...
    if request.method == "GET" and not request.is_ajax():
        ...
        return render(request, 'inventory/saftey_stock.html', context=context)
    else:
        partnames = {}
        try:
            warehouse = request.GET.get("warehouse")
            if warehouse == "all_warehouses":
                partnames = calculate_stock(
                    warehouse=None, safety_stock="safety"
                )
            else:
                warehouse_obj = Warehouse.objects.get(warehouse_code=warehouse)
                partnames = calculate_stock(
                    warehouse=warehouse_obj, safety_stock="safety"
                )
        except Warehouse.DoesNotExist as e:
            return JsonResponse({"success": False,
                                 "error": str(e)}, status=404)
